...
import { Star, Tv, ChevronLeft, ChevronRight, Menu, X, Search, Settings, LogIn, LogOut, Film } from 'lucide-react';
...
const TVCard = memo(function TVCard({ show }: { show: TVShow }) {
  const href = `/tv/${show.tmdbId || show.id}`;
  
  return (
    <Link href={href}>
      <Card className="bg-[#1a1e2e] border-0 overflow-hidden hover:scale-105 transition-transform cursor-pointer">
        <div className="relative aspect-[2/3]">
          <img src={show.poster || '/placeholder-poster.jpg'} alt={show.title} className="w-full h-full object-cover" />
          <div className="absolute top-1 right-1 bg-black/80 rounded px-1.5 py-0.5 flex items-center gap-1">
            <Star className="w-3 h-3 text-yellow-400 fill-yellow-400" />
            <span className="text-xs">{show.rating?.toFixed(1) || 'N/A'}</span>
          </div>
          {show.seasons ? (
            <div className="absolute top-1 left-1 bg-red-600 rounded px-1.5 py-0.5 text-xs font-bold">{show.seasons} Season{show.seasons > 1 ? 's' : ''}</div>
          ) : show.quality && (
            <div className="absolute top-1 left-1 bg-red-600 rounded px-1.5 py-0.5 text-xs font-bold">{show.quality}</div>
          )}
        </div>
        <CardContent className="p-2">
          ...
          <p className="text-xs text-gray-500 truncate">{getGenreNames(Array.isArray(show.genres) ? show.genres : [])}</p>
        </CardContent>
      </Card>
    </Link>
  );
...
export default function TVSeriesPage() {
  ...
  const fetchTVShows = async () => {
    setIsLoading(true);
    try {
      const res = await fetch(`/api/tv-shows?source=tmdb&page=${page}&limit=18`, { cache: 'no-store' });
        ...
        {menuOpen && (
          <div className="absolute top-full left-0 right-0 bg-[#12151c] border-b border-red-900/30 py-4 px-4 shadow-xl z-50">
            <nav className="container mx-auto flex flex-col gap-2">
              ...
              <Link href="/movies" className="p-3 hover:bg-red-900/20 rounded-lg">Movies</Link>
              <Link href="/tv-series" className="p-3 hover:bg-red-900/20 rounded-lg text-red-400">TV Series</Link>
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-red-400 flex items-center gap-2">
            <Tv className="w-6 h-6" /> TV Series
          </h1>
        </div>
        ...
        {isLoading ? (
          ...
          <>
            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-3">
              {tvShows.map((s) => <TVCard key={s.id} show={s} />)}
            </div>
